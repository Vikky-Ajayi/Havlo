...
export function CustomOffersAccess() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api.customOffersAccessConsume(token)
      .then((response) => {
        if (cancelled) return;
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F5F6F7', padding: 24 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 42, height: 42, borderRadius: '50%', border: '3px solid rgba(0,0,0,0.10)', borderTopColor: '#000000', animation: 'co-access-spin 0.8s linear infinite', margin: '0 auto 16px' }} />
          ...
          <p style={{ margin: 0, color: '#555555', fontSize: 15 }}>Checking your Custom Offers access link.</p>
          <style>{`@keyframes co-access-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
        </div>
      </div>
    );
  }
